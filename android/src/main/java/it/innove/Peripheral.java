...
public class Peripheral extends BluetoothGattCallback {
	...
	public void disconnect(boolean force, int status) {
	...
	public void readFromStream(byte[] buffer, Callback callback) {
		...
		try {
			...
			callback.invoke(null, BleManager.bytesToWritableArray(buffer));
		} catch(IOException ex) {
			callback.invoke("Couldn't be able to read from L2cap channel");
		}
	}

	public void writeToStream(byte[] data, Callback callback) {
