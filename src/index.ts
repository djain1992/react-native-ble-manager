...
class BleManager {
  ...
  openL2capChannel(peripheralId: string, psm: number) {
    return new Promise<void>((fulfill, reject) => {
      bleManager.openL2capChannel(peripheralId, psm, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  readFromStream(peripheralId: string, byteSize: number) {
    return new Promise<number[]>((fulfill, reject) => {
      bleManager.readFromStream(peripheralId, byteSize, (error: string | null, data: number[]) => {
        if (error) {
          reject(error);
        } else {
          fulfill(data);
  ...
  writeToStream(peripheralId: string, data: number[]) {
    return new Promise<void>((fulfill, reject) => {
      bleManager.writeToStream(peripheralId, data, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  closeL2capChannel(peripheralId: string) {
    return new Promise<void>((fulfill, reject) => {
      bleManager.closeL2capChannel(peripheralId, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
